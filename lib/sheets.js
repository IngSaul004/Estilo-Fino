...
export async function appendTransaction({ fecha, tipo, categoria, descripcion, monto }) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      // OJO: el apóstrofo inicial ("'" + fecha) fuerza a Google Sheets a
      // guardar la fecha como TEXTO plano ("2026-08-14"), en vez de dejar
      // que la interprete y la convierta a un valor de fecha real que luego
      // se lee reformateado (ej. "14/8/2026"). Ese reformateo era la causa
      // de que los totales y las gráficas siempre dieran $0.00: dateHelpers.js
      // no lograba parsear la fecha reformateada. El apóstrofo no aparece
      // cuando vuelves a leer la celda.
